...
export class CondominioArquivoRepository {
    ...
    async load() {
        let request = await fetch(
            `https://www.roccaimob.com.br/_admin/js-api.php`,
            {
                ...
                body: new URLSearchParams({
                    method: "getAll",
                    "params[0]":
                        "select * from condominiogrupoarquivo where deleted = 0",
                }),
                headers: {
        ...
        if (!request.ok) {
            throw new Error(
                "Não foi possível carregar os arquivos dos condominios",
            );
        }
    ...
    async getByCondominioId(
        condominio_id: number,
    ): Promise<CondominioArquivo[]> {
        await this.loading;
