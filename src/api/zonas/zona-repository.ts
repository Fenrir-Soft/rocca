    ...
    async load() {
        let request = await fetch(`https://www.roccaimob.com.br/_admin/js-api.php`, {
            ...
            body: new URLSearchParams({
                method: 'getAll',
                'params[0]': 'select id, slug, cidade_slug, titulo, ordenamento from zona where deleted = 0 ORDER BY ordenamento DESC'
            }),
            headers: {
        ...
        let rows = response.data as { id: string, slug: string, cidade_slug: string, titulo: string, ordenamento: string }[]
        let ids = rows.map(({ id }) => id)

        let request2 = await fetch(`https://www.roccaimob.com.br/_admin/js-api.php`, {
            method: 'POST',
            body: new URLSearchParams({
                method: 'getAll',
        ...
        let rows2 = response2.data as { id: string, slug: string, zona_id: string }[]
        
        for (let { id, slug, cidade_slug, titulo, ordenamento } of rows) {
            let bairros_slugs = rows2.filter((row) => Number(row.zona_id) === Number(id)).map(({ slug }) => slug)
